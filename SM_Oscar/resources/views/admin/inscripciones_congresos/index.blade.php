...
@section('title', 'Inscripciones a Congresos — UIM')

@section('admin_content')
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
        <div>
            <h1 class="h4 mb-0 text-body" style="font-weight: 700;">Inscripciones a Congresos</h1>
            <p class="text-body-secondary small mb-0">Listado de personas inscritas a los congresos de investigación.</p>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm mb-4" style="border-radius: 16px;">
        <div class="card-body p-3">
            <form method="GET" action="{{ route('admin.inscripciones_congresos.index') }}" class="d-flex flex-column flex-md-row align-items-md-center gap-2">
                <label class="fw-semibold small text-body-secondary mb-0 flex-shrink-0">Filtrar por congreso:</label>
                <select name="congreso_id" class="form-select form-select-sm" style="max-width: 350px;">
                    <option value="">— Todos los congresos —</option>
                    @foreach($congresos as $c)
                        <option value="{{ $c->id }}" {{ request('congreso_id') == $c->id ? 'selected' : '' }}>
                            {{ $c->titulo }}
                        </option>
                    @endforeach
                </select>
                ...
                @if(request('congreso_id'))
                    <a href="{{ route('admin.inscripciones_congresos.index') }}" class="btn btn-sm btn-outline-secondary">
            ...
            <table class="table table-hover align-middle mb-0">
                <thead style="background-color: #f8f9fa;">
                    <tr>
                        ...
                        <th class="py-3" style="color: var(--unam-azul); font-weight: 600; font-size: 0.82rem; text-transform: uppercase;">Congreso</th>
                    ...
                    @forelse($inscripciones as $inscripcion)
                        <tr>
                            ...
                            <td class="py-3 small text-body" style="max-width: 200px;">
                                {{ $inscripcion->congreso?->titulo ?? 'N/A' }}
                            </td>
                            <td class="py-3">
                            ...
                            <td class="px-4 py-3 text-end">
                                <div class="d-flex justify-content-end gap-2">
                                    ...
                                    {{-- Eliminar --}}
                                    <form action="{{ route('admin.inscripciones_congresos.destroy', $inscripcion) }}" method="POST" class="d-inline"
                                          onsubmit="return confirm('¿Eliminar esta inscripción al congreso?')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-5 text-body-secondary">
                                ...
                                No hay inscripciones registradas a congresos.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
