...
<section class="py-5 bg-surface-container-lowest">
    <div class="container py-4">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-end mb-5 gap-3">
            <div>
                <span
                    class="text-secondary-uim fw-bold tracking-widest small text-uppercase mb-3 d-block">{{ $s['noticias_etiqueta'] ?? 'Actualidad' }}</span>
                <h2 class="font-headline display-6 text-primary-uim fw-bold mb-0">
                    {{ $s['noticias_titulo'] ?? 'Últimas Noticias y Eventos' }}
                </h2>
            </div>
        </div>
                ...
                <article
                    ...
                    <div class="p-2 p-md-4 d-flex flex-column flex-grow-1">
                        <div class="d-flex align-items-center gap-2 mb-2">
                            <span
                                class="bg-blue-50-uim text-primary-uim fw-bold text-uppercase tracking-widest px-2 py-1 rounded-pill d-none d-sm-inline-block"
                                style="font-size: 0.6rem;">{{ $s['noticia1_categoria'] ?? 'Seminario' }}</span>
                            <span class="text-outline-uim font-label"
                                style="font-size: 0.65rem;">{{ $s['noticia1_fecha'] ?? 'Oct 24' }}</span>
                        </div>
                        <h3 class="font-headline fs-6 fw-bold text-primary-uim mb-2"
                            style="font-size: 0.85rem !important;">
                            {{ $s['noticia1_titulo'] ?? 'Nuevas perspectivas en la investigación 2024' }}
                        </h3>
                        <p class="text-on-surface-variant small mb-3 line-clamp-2 d-none d-sm-block">
                            {{ $s['noticia1_resumen'] ?? 'Se invita a la comunidad académica a participar en el ciclo de conferencias...' }}
                        </p>
            ...
            <div class="col-6 col-md-6 col-lg-4">
                <article
                    ...
                    <div class="p-2 p-md-4 d-flex flex-column flex-grow-1">
                        <div class="d-flex align-items-center gap-2 mb-2">
                            <span
                                class="bg-amber-50-uim text-secondary-uim fw-bold text-uppercase tracking-widest px-2 py-1 rounded-pill d-none d-sm-inline-block"
                                style="font-size: 0.6rem;">{{ $s['noticia2_categoria'] ?? 'Publicación' }}</span>
                            <span class="text-outline-uim font-label"
                                style="font-size: 0.65rem;">{{ $s['noticia2_fecha'] ?? 'Oct 12' }}</span>
                        </div>
                        <h3 class="font-headline fs-6 fw-bold text-primary-uim mb-2"
                            style="font-size: 0.85rem !important;">
                            {{ $s['noticia2_titulo'] ?? 'Presentación Revista FIGURAS: Invierno' }}
                        </h3>
                        <p class="text-on-surface-variant small mb-3 line-clamp-2 d-none d-sm-block">
                            {{ $s['noticia2_resumen'] ?? 'Explora los artículos más recientes sobre humanidades digitales...' }}
                        </p>
                        <a class="mt-auto text-secondary-uim fw-bold text-uppercase tracking-widest text-decoration-none d-flex align-items-center gap-1"
                ...
                <article
                    ...
                    <div class="p-2 p-md-4 d-flex flex-column flex-grow-1">
                        <div class="d-flex align-items-center gap-2 mb-2">
                            ...
                            <span class="text-outline-uim font-label"
                                style="font-size: 0.65rem;">{{ $s['noticia3_fecha'] ?? 'Sep 28' }}</span>
                        </div>
                        <h3 class="font-headline fs-6 fw-bold text-primary-uim mb-2"
                            style="font-size: 0.85rem !important;">
                            {{ $s['noticia3_titulo'] ?? 'Alianza con institutos internacionales' }}
                        </h3>
                        <p class="text-on-surface-variant small mb-3 line-clamp-2 d-none d-sm-block">
                            {{ $s['noticia3_resumen'] ?? 'UIMA firma convenio de colaboración con universidades europeas...' }}
                        </p>
                        <a class="mt-auto text-secondary-uim fw-bold text-uppercase tracking-widest text-decoration-none d-flex align-items-center gap-1"
